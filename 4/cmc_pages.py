...
def get_page_data(html):
    ...
    for tr in trs:
        ...
        try:
            url = f"https://coinmarketcap.com{tds[1].find('a', class_='currency-name-container').get('href')}"
...
def main():
    ...
    while True:
        ...
        try:
            pattern = 'Next'
            url = f"https://coinmarketcap.com/{soup.find('ul', class_='pagination').find('a', text=re.compile(pattern)).get('href')}"
